...
config = {"mol": 'all', "P_spectra_dim": 101, 
     "input_csv_path": './data/all/combined_x.csv',
     "label_csv_path": './data/all/combined_y.csv',
     "output_plot_dir": './pred/all'} #optional

file_paths = ['./data/P_mol_spectra/mol1-p.xlsx', 
              './data/P_mol_spectra/mol2-p.xlsx', 
              './data/P_mol_spectra/mol3-p.xlsx']
...
test_size = 0.1
k_folds = 10
autoencoder_checkpoint = './ckpt/autoencoder/1500.pth'
ckpt_dir = "./ckpt/model"
results_file_path = './result/baseline_svr_results_summary.csv'
...
def main():
    ...
    for random_seed in random_seeds:       
        ...
        for fold, (train_index, test_index) in enumerate(kf.split(inputs)):
            ...
            # Train SVR for each output dimension
            svr_models = []
            for i in range(test_targets.shape[1]):  # Iterate over each output dimension
                svr_model = SVR(kernel='rbf', C=1e3, gamma=0.1)
                svr_model.fit(train_features, train_targets[:, i])
                svr_models.append(svr_model)
            
            # Predict on test set
            predictions = np.zeros((test_features.shape[0], test_targets.shape[1]))
            for i in range(test_targets.shape[1]):
                predictions[:, i] = svr_models[i].predict(test_features)
